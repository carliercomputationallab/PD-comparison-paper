...
for i,pfile in enumerate(patientlist):
    ...
    tp_ond = datafile.loc[pfile, "TP_OND"] #total protein overnight dwell in g/L
    tp_ff = datafile.loc[pfile, "TP_FF"]
    ...
    #calculate MTAC based on Garred model
    mtac = V.mean()/t*np.log(
        pd.to_numeric((V[0]*(cp-df_cd.loc[0]))/
        (V[240]*(cp-df_cd.loc[240]))))
    print(pfile, cp)
    ...
    for solute in solutes:
        for ind in list(df_cd.index)[::-1]:
            mtac[solute] = V.mean()/ind*np.log(
                (pow(V[0], 1)*(cp[solute]-df_cd[solute].loc[0]))/
                (pow(V[ind], 1)*(cp[solute]-df_cd[solute].loc[ind])))
            if np.isfinite(mtac[solute]):
                break

    MTAC_G = pd.concat([MTAC_G,mtac], axis=1)
    ...
    for t in range(1,241):
        predicted_cd.loc[t] = cp-(cp-predicted_cd.loc[0])*(V[0]/V[t])*np.exp(-mtac/V.mean()*(t))
    predicted_cd.to_excel(f'predicted_cd/GM_{pfile}.xlsx')       
    #error calculation
    ...
    # with pd.ExcelWriter('fittedPS/predicted_cd_GM_PDPC_754.xlsx', engine='openpyxl') as writer:
    #     predicted_cd.to_excel(writer) 
    
    
MTAC_G = MTAC_G.T
MTAC_G = MTAC_G[1:]
MTAC_G.index = patientlist
MTAC_G.columns = ['MTAC_urea', 'MTAC_crea', 'MTAC_glu', 'MTAC_sodium']
MTAC_G['obj_fn'] = obj_fn
MTAC_G['comp-time'] = comp_time
print(MTAC_G)


with pd.ExcelWriter('fittedPS/fittedPS_GM.xlsx', engine='openpyxl') as writer:
    MTAC_G.to_excel(writer)    
