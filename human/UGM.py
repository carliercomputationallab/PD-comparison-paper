# -*- coding: utf-8 -*-
"""
Created on Sun Feb 11 12:37:45 2024
...
def calculate_penalty(x):
    ...
    #penalty for having sieving coefficient beyond 0 and 1
    penalty = 0
    for param in x[4:7]:
        if param < 0:
            penalty += param ** 2
...
def objective_fn(x, predicted_cd, Cp, V, df_cd):
    ...
    #extra penalty for unrealistic parameter fitting
    penalty = calculate_penalty(x)
...
# the differential equations Graff model
def compute(cd, t, x, predicted_cd, Cp, V, df_cd):
    ...
    MTAC = np.array(x[0:4]) #ml/min
    fct = np.array([1, 1, 1, 0.94]) #dimensionless
    SiCo = np.array([x[4], x[5], 0, x[6]]) #SiCo dimensionless
    SiCo = np.around(SiCo, decimals=4) #to reduce no of significant digits
    L = 0.3 # lymphatic flow rate ml/min
    QU = V[t+1] - V[t] + L #UF rate ml/min
    ...
    MC = cp-f*(cp-cd) #intramembrane concentration mmol/L
    Cl = cp if L < 0 else cd #depending on direction of flow, choose concentration that lymphatically flows mmol/L
...
def multiprocessing_func(pfile):
    ...
    st = time.time()
    Nx = 7  #MTAC Urea, Crea, Glucose and sodium and Sieving coeff Urea, Crea, Na
    ...
    obj_fn = []
    print(pfile, flush = True)
    
    for var in range(10):
        
        
        x0 = np.random.randint(1, 21, size=Nx)
        lbound = [0.01, 0.01, 0.01, 0.01,  0, 0, 0]
        ubound = [200, 200, 200, 200, 1, 1, 1] 
...
if __name__ == '__main__':
    ...
    for i in range(len(result_list)):
        ...
        print('Writing predicted cd')
        predicted_cd.to_excel(f'predicted_cd/UGM_{pfile}.xlsx') 
        
    cols = ['MTAC_urea', 'MTAC_crea','MTAC_glu',
            
            'MTAC_sodium', 'sico_urea', 'sico_crea', 'sico_sodium'] 
    ...
    print('Writing fitted PS')
    with pd.ExcelWriter('fittedPS/fittedPS_UGM.xlsx', engine='openpyxl') as writer:
        PS.to_excel(writer) 