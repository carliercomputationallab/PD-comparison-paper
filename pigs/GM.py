...
for i,pfile in enumerate(patientlist):  
    ...
    mtac = V.mean()/t*np.log(
        (V[0]*(df_cp.mean()-df_cd.loc[0]))/
        (V[240]*(df_cp.mean()-df_cd.loc[240])))
    
    for solute in solutes:
        for ind in list(df_cd.index)[::-1]:
            mtac[solute] = V.mean()/ind*np.log(
                (V[0]*(df_cp[solute].mean()-df_cd[solute].loc[0]))/
                (V[ind]*(df_cp[solute].mean()-df_cd[solute].loc[ind])))
    ...
    for t in range(1,241):
        predicted_cd.loc[t] = df_cp.mean()-(df_cp.mean()-predicted_cd.loc[0])*(V[0]/V[t])*np.exp(-mtac/V.mean()*t)
        
    predicted_cd.to_excel(f'predicted_cd/GM_{patientlist[i][28:]}.xlsx')    
...
MTAC_G.columns = ['MTAC_urea', 'MTAC_crea','MTAC_sodium', 'MTAC_phosphate','MTAC_glu', 'MTAC_potassium'] 
MTAC_G['obj_fn'] = obj_fn
MTAC_G['comp time'] = time_Garred
print(MTAC_G)
