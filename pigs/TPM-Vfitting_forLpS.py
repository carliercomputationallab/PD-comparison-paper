...
def multiprocessing_func(pfile):
    ...
    #note here V is not converted into ml because another input file UFvalues.py is used 
...
if __name__ == '__main__':
    ...
    for i in range(len(result_list)):
        ...
        UF[i] = result_list[i][2]
        
        
    #%% 
    LPS = pd.DataFrame(columns = ['patient', 'LpS', 'obj fn', 'UF'], index = range(0, len(patientlist)))
    for i in range(len(patientlist)):
        LPS.loc[i, 'patient'] = patientlist[i][28:]
        LPS.loc[i, 'LpS'] = OV[i]
        LPS.loc[i, 'obj fn'] = OF[i]
        LPS.loc[i, 'UF'] = UF[i]
        
    with pd.ExcelWriter('specific_pig/predicted_files/correctedLPS_TPM.xlsx', engine='openpyxl') as writer:
        LPS.to_excel(writer)
    #%%
